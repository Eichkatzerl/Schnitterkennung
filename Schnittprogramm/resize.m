...
folder = fileparts(which('MustererkennungTestdaten.mp4')); % Determine where demo folder is (works with all versions).
% Pick one of the two demo movies shipped with the Image Processing Toolbox.
% Comment out the other one.
inputFullFileName = fullfile(folder, 'MustererkennungTestdaten.mp4');
...
outputFullFileName = fullfile(pwd, 'MustererkennungTestdaten2.avi');
outputVideoWriterObject = VideoWriter(outputFullFileName);
open(outputVideoWriterObject);
% Specify the output video size.
shrinkFactor = 8; % Shrink by a factor of 4 in both directions.
